...
// ---------------------------------------------------------------------------
// Local constants (self-contained, no imports)
// ---------------------------------------------------------------------------

const DEFAULT_LEAGUE = 'nfl';
const SUPPORTED_LEAGUES = ['nfl', 'nba', 'mlb', 'nhl'];
const API_BASE = '/api';
...
// ---------------------------------------------------------------------------
// Module-level mutable state (replaces TeamsUtil object properties)
// ---------------------------------------------------------------------------
...
export function normalizeTeamString(value) {
    if (!value) return '';
...
export function buildTeamIndex(league) {
    if (teamIndex[league]) {
        return;
    }
    ...
    Object.values(teams).forEach(team => {
        ...
        const nameKey = normalizeTeamString(team.name);
        if (nameKey) {
            index.byName[nameKey] = team;
            const aliases = LOGO_NAME_ALIASES[league]?.[nameKey] || [];
            aliases.forEach(alias => {
                const aliasKey = normalizeTeamString(alias);
                if (aliasKey) {
                    index.byName[aliasKey] = team;
                }
            });
        }
        const cityKey = normalizeTeamString(team.city);
        let nickname = '';
        if (cityKey) {
            const nameLower = team.name.toLowerCase();
            const cityLower = team.city.toLowerCase();
            if (nameLower.startsWith(cityLower)) {
                nickname = normalizeTeamString(team.name.slice(team.city.length).trim());
            }
        }
        if (nickname) {
    ...
    teamIndex[league] = index;
}

export function getTeamByAbbreviation(abbreviation, league = DEFAULT_LEAGUE) {
    if (!abbreviation) return null;
    buildTeamIndex(league);
    return teamIndex[league]?.byAbbr[abbreviation.toUpperCase()] || null;
}

export function getTeamByName(name, league = DEFAULT_LEAGUE) {
    if (!name) return null;
    buildTeamIndex(league);
    const normalized = normalizeTeamString(name);
    return teamIndex[league]?.byName[normalized] || null;
}

export function getMatchCandidates(name, league = DEFAULT_LEAGUE) {
    const normalizedName = normalizeTeamString(name);
    if (!normalizedName) return [];
    const teams = getAllTeams(league);
    const candidates = [];

    teams.forEach(team => {
        const teamName = normalizeTeamString(team.name);
        if (!teamName) return;
        let score = 0;
        ...
        const cityName = normalizeTeamString(team.city);
...
export function resolveTeam(rawTeam, league = DEFAULT_LEAGUE) {
    if (!rawTeam) return null;
    let resolved = rawTeam.id
        ? getTeam(rawTeam.id, league)
        : (rawTeam.abbreviation
            ? getTeamByAbbreviation(rawTeam.abbreviation, league)
            : (rawTeam.name ? getTeamByName(rawTeam.name, league) : null));

    if (!resolved && rawTeam.name) {
        resolved = getMatchCandidates(rawTeam.name, league)[0]?.team || null;
    }
    ...
    return applyTeamColors(merged, league);
}

/**
 * Get all teams as an array, sorted alphabetically by name
 */
export function getAllTeams(league = DEFAULT_LEAGUE) {
    const teams = TEAMS_BY_LEAGUE[league] || {};
 ...
 */
export function getTeam(id, league = DEFAULT_LEAGUE) {
    const teams = TEAMS_BY_LEAGUE[league] || {};
 ...
 */
export function getTeamBySlug(slug, league = DEFAULT_LEAGUE) {
    const teams = TEAMS_BY_LEAGUE[league] || {};
 ...
 * @param {string|null} league - League to search in, or null/'all' for all leagues
 * @returns {Object} Team info
 */
export function parseTeamsFromTitle(title, league = null) {
    const result = {
        away: null,
        home: null,
        awayId: null,
    ...
    for (const leagueKey of leaguesToCheck) {
        const awayCandidates = getMatchCandidates(team1Name, leagueKey);
        const homeCandidates = getMatchCandidates(team2Name, leagueKey);
 ...
 */
export function getTeamLogo(team, league = DEFAULT_LEAGUE) {
    if (!team) {
        return null;
    }
    const resolvedLeague = resolveLogoLeague(league, team);
    const directLogo = getDirectLogo(team);
    if (!resolvedLeague) {
        return directLogo;
    }
    const espnLogo = getEspnLogo(team, resolvedLeague);
    if (espnLogo) {
        return espnLogo;
    }
...
export function getDirectLogo(team) {
    if (!team) {
        return null;
    }
...
export function getEspnLogo(team, league) {
    ...
    if (abbr) {
        const apiLogo = logoMap[league]?.[abbr];
        if (apiLogo) {
            return apiLogo;
        }
        const aliasAbbr = LOGO_ABBR_ALIASES[league]?.[abbr];
        if (aliasAbbr) {
            const aliasLogo = logoMap[league]?.[aliasAbbr];
            if (aliasLogo) {
                return aliasLogo;
            }
        }
    }
    const nameLogo = getLogoByName(team, league);
    if (nameLogo) {
        return nameLogo;
    }
    if (!abbr) {
...
export function resolveLogoLeague(league, team) {
    if (league && logoMap[league]) {
        return league;
    }
    const teamLeague = team?.league;
    if (teamLeague && logoMap[teamLeague]) {
        return teamLeague;
    }
    return null;
}

export function getLogoByName(team, league) {
    const candidates = getLogoNameCandidates(team, league);
    for (const nameKey of candidates) {
        const logo = logoNameMap[league]?.[nameKey];
        if (logo) {
            return logo;
        }
    }
...
export function getLogoNameCandidates(team, league) {
    const names = new Set();
    const add = (value) => {
        const normalized = normalizeTeamString(value);
        if (normalized) {
    ...
    const primary = normalizeTeamString(team?.name);
...
export function getColorByAbbreviation(abbr, league) {
    if (!abbr) return null;
    const key = abbr.toUpperCase();
    return colorMap[league]?.[key] || null;
}

export function getColorByName(name, league) {
    if (!name) return null;
    const key = normalizeTeamString(name);
    return colorNameMap[league]?.[key] || null;
}

export function applyTeamColors(team, league) {
    if (!team) return team;
    const existing = team.color || team.primaryColor || team.teamColor;
    if (existing) return team;

    const fromAbbr = getColorByAbbreviation(team.abbreviation, league);
    const fromName = getColorByName(team.name, league);
 ...
 * @param {string} league - League key or 'all'
 * @returns {Promise<void>}
 */
export async function preloadLogos(league = 'all') {
    const leaguesToLoad = league === 'all'
        ? SUPPORTED_LEAGUES
        : [league];

    await Promise.all(leaguesToLoad.map(leagueKey => preloadLogosForLeague(leagueKey)));
}

export async function preloadLogosForLeague(league) {
    if (!SUPPORTED_LEAGUES.includes(league)) {
        return;
    }
    if (logosLoaded[league]) {
        return;
    }
    if (logosPromise[league]) {
        return logosPromise[league];
    }

    logosPromise[league] = (async () => {
        try {
            const shouldForce = Object.keys(colorMap[league] || {}).length === 0;
            const url = `${API_BASE}/teams?league=${league}${shouldForce ? '&force=1' : ''}`;
            ...
            if (Array.isArray(data.teams)) {
                data.teams.forEach(team => {
                    ...
                    nameKeys.forEach(value => {
                        const key = normalizeTeamString(value);
                        if (key && team.logo) {
                            logoNameMap[league][key] = team.logo;
                        }
                    });

                    if (abbr) {
                        colorMap[league][abbr] = {
 ...
 */
export function getTeamsByDivision(league = DEFAULT_LEAGUE) {
    const grouped = {};
    const teams = TEAMS_BY_LEAGUE[league] || {};
    Object.values(teams).forEach(team => {
...
/**
 * Get all teams for a given league (alias for getAllTeams)
 */
export function getTeamsForLeague(league) {
    return getAllTeams(league);
}
